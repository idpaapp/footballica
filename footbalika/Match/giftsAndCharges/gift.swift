...
public class gift : menuView {
    ...
    public override func awakeFromNib() {
        ...
        giftsNumbers.append("\((gameDataModel.loadGameData?.response?.giftRewards?.sign_up!)!)")
        }

        if (login.res?.response?.mainInfo?.email_connected!)! != "1" {
        giftsImages.append("google_plus")
        giftsTitles.append("اتصال به حساب گوگل")
        giftsNumbers.append("\((gameDataModel.loadGameData?.response?.giftRewards?.google_sign_in!)!)")
        }

        giftsImages.append("ic_bug")
        giftsTitles.append("گزارش مشکل")
        giftsNumbers.append("\((gameDataModel.loadGameData?.response?.giftRewards?.report_bug!)!)")

        giftsImages.append("ic_comment")
        giftsTitles.append("انتقاد و پیشنهاد")
        giftsNumbers.append("\((gameDataModel.loadGameData?.response?.giftRewards?.comment!)!)")
